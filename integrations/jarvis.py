"""JARVIS telemetry adapter for Autonomous Product Discovery Engine."""
from __future__ import annotations
import json, os, urllib.request
from datetime import datetime, timezone

PROJECT_ID = "autonomous_product_discovery"

def build_event(event_type: str, payload: dict) -> dict:
    now = datetime.now(timezone.utc).isoformat()
    return {"schema_version":"1.0","event_id":f"{PROJECT_ID}:{now}","project_id":PROJECT_ID,"event_type":event_type,"occurred_at":now,"payload":payload}
